...
import { Download, Upload, Database, FileJson, CloudRain, Loader2, CheckCircle, Share2, MessageCircle, Copy } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
         ...
         <div className="relative z-10">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-[2.5rem] p-8 border border-white/10 max-w-xl">
               <p className="text-sm font-medium mb-6 opacity-80 leading-relaxed">
                  Generate a Sync Key for your remote drivers. They just need to enter this code on their phone to receive your updates.
               </p>
               ...
               <div className="space-y-6">
                  <div className="flex flex-col md:flex-row gap-6 items-center">
                    <div className="flex-1 w-full">
                      ...
                      <div className="bg-black/20 rounded-2xl py-5 text-center text-xl md:text-2xl font-black tracking-widest border border-white/10 flex items-center justify-center gap-4 group/key">
                          {syncCode || '---'}
                          {syncCode && (
                    ...
                    >
                      {isGenerating ? <Loader2 className="animate-spin" size={18} /> : (syncCode ? 'Reset Key' : 'Create Sync Key')}
                    </button>
                  </div>
                  ...
                  {syncCode && (
                    <div className="flex flex-col sm:flex-row gap-4">
                       <button 
                        ...
                        className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white py-4 rounded-2xl flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-widest transition-all shadow-lg"
                       >
                          <MessageCircle size={18} /> Send to Driver (WhatsApp)
                       </button>
                    </div>
                  )}
               </div>
               ...
               {syncCode && (
                 <div className="mt-6 flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-emerald-300">
                   <CheckCircle size={14} />
                   <span>System linked to Cloud. Share the key with remote staff.</span>
                 </div>
               )}
            </div>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
            ...
            <button onClick={handleExport} className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 py-4 rounded-2xl font-bold text-xs shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 hover:bg-slate-900 hover:text-white transition-all flex items-center justify-center gap-3">
          ...
          <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
            ...
            <button onClick={() => fileInputRef.current?.click()} className="w-full bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 py-4 rounded-2xl font-bold text-xs shadow-sm ring-1 ring-amber-200 dark:ring-amber-900/50 hover:bg-amber-600 hover:text-white transition-all flex items-center justify-center gap-3">
